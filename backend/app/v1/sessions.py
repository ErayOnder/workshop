...
from ..crud.feedback import create_feedback_event
...
# --------------------------------------------------------------------------- #
# POST /v1/sessions/{session_id}/feedback — submit feedback + get next round  #
# --------------------------------------------------------------------------- #
...
async def submit_feedback(
    ...
    # Determine exploit mode (consecutive_likes not used in V1; avoid lazy-loading session.feedback_events in async context)
    consecutive_likes = _count_consecutive_likes(session_id, db)
    save_ever_pressed = body.action == "save"
    exploit = should_exploit(session.round_number, save_ever_pressed, 0)
    ...
    configs = generate_three_configs(new_scores, round_number, None, exploit_mode=exploit)
    prompts = [compile_prompt(c) for c in configs]
